...
SELECT livros.nome_Livro AS Livro, autores.nome_Autor AS Autor,
 livros.preco_Livro AS Valor
FROM livros
INNER JOIN autores
ON livros.ID_Autor = autores.ID_Autor;
...
alter table consumo add constraint
 vei_consumo foreign key (placa_fk)
...
select m.marca, v.modelo, v.cor, avg(c.kmRodado) as
 'media final'
from marca m, veiculos v, consumo c
...
select * from vw_exemplo4;

create view qtdVeiculosMarca as
...
select * from qtdVeiculosMarca;

select marca from qtdVeiculosMarca;
...
insert into viewmarca(marcaveiculo) values ('Jipe');
...
update viewmarca set marcaveiculo = 'Jaguar' where numeromarca = 4;
